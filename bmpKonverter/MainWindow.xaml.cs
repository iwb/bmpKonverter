    ...
    public partial class MainWindow : Window
    {
            ...
            if (_cts == null) // Berechnung starten
            {
                ...
                var task = Task.Factory.StartNew(() =>
                {
                    ...
                    Parallel.ForEach(files, options, (filepath) =>
                    {
                        ...
                        string dest = System.IO.Path.Combine(outputdir, filename + ".png");
                        if (!File.Exists(dest))
        ...
        private void ResetGUI(Task t)
        {
            ...
            if (t.Status == TaskStatus.RanToCompletion)
                StatusLbl.Content = string.Format("{0} Dateien wurden erfolgreich konvertiert. ({1:0.00 Bilder/s})", filecount, filecount / seconds);
            else
